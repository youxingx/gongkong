...
def main():
	...
	# 创建一个GPS对象，通过串口对gps数据进行处理
	Serial_GPS=serials.Serial_GPS('/dev/ttymxc4')
	# 创建一个角度对象，通过串口对角度传感器数据进行处理
	Serial_Angle=serials.Serial_Angle('/dev/ttyHBB1')
	# 创建一个短程雷达左对象，通过串口对左边的短程雷达数据进行处理
	# Serial_LeftRadar=serials.Serial_LeftRadar(13)
	# 创建一个长程雷达对象，通过串口对长程雷达数据进行处理
	Serial_LongRadar=serials.Serial_LongRadar('/dev/ttyHBB0')
	# 创建一个短程雷达右对象，通过串口对右边的短程雷达数据进行处理
	# Serial_RightRadar=serials.Serial_RightRadar(15)
